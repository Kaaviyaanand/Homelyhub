...
import { fetchUserBookings,fetchBookingDettails } from "../../Store/Booking/booking-action";
...
const MyBookings = () => {
  ...
  const handleBookingClick=(bookingId)=>{
    dispatch(fetchUserBookings(bookingId));
    navigate(`/user/booking/${bookingId}`);

  };
  if (bookings.length=== 0 && !loading){
    return <div>MY Bookings Are not avalilable...</div>
  }
  return (
    <>
      <ProgressSteps />
